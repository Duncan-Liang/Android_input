...
static struct {
    ...
    jmethodID interceptMotionWhenProximityEnabled;
...
void NativeInputManager::interceptMotionBeforeQueueing(nsecs_t when, uint32_t& policyFlags) {
    ...
    //add by mohongwu@wind-mobi.com  2016/7/27 begin
#ifdef WIND_DEF_ASUS_AVIOD_TOUCH
    JNIEnv* env = jniEnv();
    jint wmActions = env->CallIntMethod(mServiceObj,
    gServiceClassInfo.interceptMotionWhenProximityEnabled,
    when, policyFlags);
    if (checkAndClearExceptionFromCallback(env,
    "interceptMotionWhenProximityEnabled")) {
    wmActions = 0;
    ALOGI("error do jni in interceptMotionWhenProximityEnabled");
    }
    ALOGI("bef ALOD wmActions= %d",wmActions);
    handleInterceptActions(wmActions, when, /*byref*/ policyFlags);
    //if this is intercept in phonewindowmanager
...
int register_android_server_InputManager(JNIEnv* env) {
    ...
    //add by mohongwu@wind-mobi.com  2016/7/27 begin
    GET_METHOD_ID(gServiceClassInfo.interceptMotionWhenProximityEnabled, clazz,
            "interceptMotionWhenProximityEnabled", "(JI)I");
    //add by mohongwu@wind-mobi.com  2016/7/27 end
