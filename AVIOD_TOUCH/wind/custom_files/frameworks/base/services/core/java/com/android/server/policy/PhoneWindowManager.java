...
// wangyan@wind-mobi.com add 2016/07/22 Feature #110195 start
import android.hardware.Sensor;
import android.hardware.SensorEvent;
import android.hardware.SensorEventListener;
import android.hardware.SensorManager;
// wangyan@wind-mobi.com add 2016/07/22 Feature #110195 end
...
//pengfugen@wind-mobi.com 2016/03/23 start
import android.telephony.TelephonyManager;
//pengfugen@wind-mobi.com 2016/03/23 end
//add by youxiaoyan for SystemUI 20160309 -s
import android.hardware.input.InputManager;
//add by youxiaoyan for SystemUI 20160309 -e

// wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -s
import com.android.server.policy.flipcover2.FlipCover2ServiceDelegate;
// wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -e
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final boolean WIND_DEF_ONE_HAND_CONTROL = SystemProperties.get("ro.wind.def.one.hand.control").equals("1");
    // pengfugen@wind-mobi.com 2016/03/24 end
    // pengfugen@wind-mobi.com 2016/05/30 start
    static final boolean WIND_DEF_MOTION_GESTURE = SystemProperties.get("ro.wind.def.motion_support").equals("1");
    // pengfugen@wind-mobi.com 2016/05/30 end
    //add by sunxiaolong@wind-mobi.com for otg reverse charging start
    private static final boolean WIND_OTG_REVERSE = SystemProperties.getBoolean("ro.wind.otg.reverse.charging",false);
    //add by sunxiaolong@wind-mobi.com for otg reverse charging end
	...
	//add by lizusheng@wind-mobi.com  20160504 -s
//    private static final boolean WIND_FINGERPRINT_SILEAD_ENABLE = SystemProperties.get("ro.wind_fingerprint_silead").equals("1");
    //add by lizusheng@wind-mobi.com  20160504 -e

    //add by mohongwu@wind-mobi.com 2016/7/27 begin
    public static boolean mShouldTurnOffTouch = false;
    //add by mohongwu@wind-mobi.com 2016/7/27 end
    /**
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    //BEGIN: John_Ho@asus.com
    // recent app key long press function support
    volatile boolean mSwitchKeyHandled;
    private int mFuncWhenLongPressAppSwitch = Settings.System.LONG_PRESSED_FUNC_DEFAULT;
    //END: John_Ho@asus.com
    //add by youxiaoyan for SystemUI 20160309 -e

	/* lifeifei@wind-mobi.com 20160407 add for 104994 begin */
    //BEGIN: Jeffrey_Chiang@asus.com Instant camera porting
    ...
    private boolean mCameraKeyDoubleClickPending = false;
    private boolean mRecordKeyDoubleClickPending = false;
    private boolean mIsLongPressInstantCameraEnabled = false;
    private PowerManager.WakeLock mInstantCameraWakeLock = null;
    private AudioManager mAudioManager = null;
	private LongPressLaunchCamera mLongPressLaunchCameraRunnable = new LongPressLaunchCamera();
    //END: Jeffrey_Chiang@asus.com
	/* lifeifei@wind-mobi.com 20160407 add for 104994 end */
	...
	/* lifeifei@wind-mobi.com 20160407 add for 104994 begin */
    //BEGIN: Jeffrey_Chiang@asus.com Instant camera porting
    private static final int MSG_HOLD_INSTANT_CAMERA_WAKE_LOCK = 4201;
    private static final int MSG_DOUBLE_CLICK_EVENT = 4202;
    private static final int MSG_LONG_PRESS_EVENT = 4203;
    private static final int MSG_KEY_UP_EVENT = 4204;
    //END: Jeffrey_Chiang@asus.com
	/* lifeifei@wind-mobi.com 20160407 add for 104994 end */
    //add by sunxiaolong@wind-mobi.com for otg reverse charging start
    private static final int OTG_PLUG_IN_OPERATION = 16;
    private static final int OTG_PLUG_OUT_OPERATION = 17;
    ...
    private static int BATTERY_LEVEL = 100;
    private static AlertDialog ReverseChargingDialog;
    private static boolean POP_OTG = false;
    //add by sunxiaolong@wind-mobi.com for otg reverse charging end

    // wangyan@wind-mobi.com add 2016/04/07 for Feature#100647 -s
    private static final boolean WIND_DEF_APP_POWERSAVER= SystemProperties.get("ro.wind_os_app_powersaver").equals("1");
    // wangyan@wind-mobi.com add 2016/04/07 for Feature#100647 -e
    ...
    private static final boolean WIND_DEF_ASUS_GESTURES = SystemProperties.get("ro.wind.def.asus.gestures").equals("1");
    private boolean mGestureEnabled = true;
    private SensorManager mSensorManager;
    private Sensor mProximityMotion;
    // wangyan@wind-mobi.com add 2016/06/24 for Feature#110195 end

    // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -s
    boolean mHasTranscoverFeature;
    ...
    //add by mohongwu@wind-mobi.com 2016/9/9 start
    private static final int MSG_GESTURE_LISTEN = 5555;
    //add by mohongwu@wind-mobi.com 2016/9/9 end
    ...
    private void setFlipCoverSysPropety(){
        try {
            ...
            android.os.SystemProperties.set("persist.asus.flipcovermode", Integer.toString(value));
        } catch (Exception e) {
            // TODO: handle exception
            Log.w(FLIPCOVER_TAG, e.toString());
        }
        ...
        try {
            ...
            if(user==null){
                ...
                if(mHasTranscoverInfoFeature){
                    result=((android.os.Build.VERSION.SDK_INT>=23)?(!user.isAdmin()):user.isSnapView());
            ...
            switch (msg.what) {
                ...
                case MSG_FLIPCOVER_UNLOCK_LOCKSCREEN:
                    if(mKeyguardDelegate!=null){
                        mKeyguardDelegate.unLockNonSecureLock();
                    }
                    break;
                // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -e
					/* lifeifei@wind-mobi.com 20160407 add for 104994 begin */
                //BEGIN: Jeffrey_Chiang@asus.com Instant camera porting
                case MSG_HOLD_INSTANT_CAMERA_WAKE_LOCK:
                    holdInstantCameraWakeLock();
                    break;
                //END: Jeffrey_Chiang@asus.com
                ...
                case MSG_DOUBLE_CLICK_EVENT:
                    ...
                    if (mCameraKeyDoubleClickPending && "hardware:camerakey".equals(doubleClickSrc)) {
                        mHandler.removeCallbacks(mLongPressLaunchCameraRunnable);
                        mCameraKeyDoubleClickPending = false;
                        mHandler.removeCallbacks(mCameraKeyDoubleClickTimeoutRunnable);
                        handleDoubleClickLaunchCamera(doubleClickSrc,doubleClickStatus,doubleKeyCode);
                    }

                    if (mRecordKeyDoubleClickPending && "hardware:camerarecordkey".equals(doubleClickSrc)) {
                        mHandler.removeCallbacks(mLongPressLaunchCameraRunnable);
                        mRecordKeyDoubleClickPending = false;
                        mHandler.removeCallbacks(mRecordKeyDoubleClickTimeoutRunnable);
                        handleDoubleClickLaunchCamera(doubleClickSrc,doubleClickStatus,doubleKeyCode);
                    }
                ...
                case MSG_KEY_UP_EVENT:
                    ...
                    if ("hardware:camerakey_up".equals(upSrc)) {
                        mHandler.removeCallbacks(mCameraKeyDoubleClickTimeoutRunnable);
                        mCameraKeyDoubleClickPending = true;
                        mHandler.postDelayed(mCameraKeyDoubleClickTimeoutRunnable,
                           ViewConfiguration.getDoubleTapTimeout());
                    }

                    if ("hardware:camerarecordkey_up".equals(upSrc)) {
                        mHandler.removeCallbacks(mRecordKeyDoubleClickTimeoutRunnable);
                        mRecordKeyDoubleClickPending = true;
                        mHandler.postDelayed(mRecordKeyDoubleClickTimeoutRunnable,
                           ViewConfiguration.getDoubleTapTimeout());
                    }
                ...
                case MSG_GESTURE_LISTEN:
                    int keyCode = ((Integer)msg.obj).intValue();
                    Slog.i(TAG, "MSG_GESTURE_LISTEN keyCode " + keyCode);
                    if(getProximitySensorData() < 5000){
                        //remove by mohongwu@wind-mobi.com 2016/9/14 start
                        //sendGestureBroadcast(keyCode);
                        //remove by mohongwu@wind-mobi.com 2016/9/14 end
                        //add by mohongwu@wind-mobi.com 2016/9/14 start
                        if(WIND_GESTURES_SUPPORTED){
                            if (mContext != null) {
                                Intent intent = new Intent("com.android.intent.gestures");
                                intent.putExtra("keyCode", keyCode);
                                mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
                            }
                        }else if (WIND_DEF_ASUS_GESTURES){
                            sendGestureBroadcast(keyCode);
                        }
                        //add by mohongwu@wind-mobi.com 2016/9/14 end
                    }
                    //add by mohongwu@wind-mobi.com 2016/9/13 start
                    mSensorManager.unregisterListener(mDistanceSensorListener);
                    //add by mohongwu@wind-mobi.com 2016/9/13 end
                    break;
                //add by mohongwu@wind-mobi.com 2016/9/9 end
                //add by sunxiaolong@wind-mobi.com for otg reverse charging start
                case OTG_PLUG_IN_OPERATION:
                    if (checkBatteryPercent() <= 20) {
                        setPowerReverseChargeEnable(false, OTG_REVERSE_CHARGING_CONTROL);
                    } else {
                        popUpDialog(mContext);
                    }
                    break;
                case OTG_PLUG_OUT_OPERATION:
                    setPowerReverseChargeEnable(false, OTG_REVERSE_CHARGING_CONTROL);
        ...
        if (!mPowerKeyHandled) {
            if (interactive) {
                ...
                if (hasLongPressOnPowerBehavior()) {
                    ...
                    //modify by mohongwu@wind-mobi 2016/8/17 begin
                    mHandler.sendMessageDelayed(msg,
                            ViewConfiguration.get(mContext).getDeviceGlobalActionKeyTimeout() + 1500);
                    //modify by mohongwu@wind-mobi.com 2016/8/17 end
                }
            } else {
    ...
    private void handleLongPressOnHome(int deviceId) {
        // wangyan@wind-mobi.com add 2016/04/07 for Feature#100647 -s
        if (WIND_DEF_APP_POWERSAVER) {
            if ("on".equals(Settings.System.getString(mContext.getContentResolver(), "powersave_super"))) {
                return;
            }
        }
        // wangyan@wind-mobi.com add 2016/04/07 for Feature#100647 -e
    ...
    private void handleDoubleTapOnHome() {
        ...
        // pengfugen@wind-mobi.com 2016/03/24 start
                ...
                && isUserSetupComplete() && WIND_DEF_ONE_HAND_CONTROL) {
            mHomeConsumed = true;
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        //add by sunxiaolong@wind-mobi.com for reverse charging start
        if(WIND_OTG_REVERSE) {
            IntentFilter batteryFilter = new IntentFilter();
            batteryFilter.addAction(Intent.ACTION_BATTERY_CHANGED);
            batteryFilter.addAction(Intent.ACTION_SCREEN_ON);
            batteryFilter.addAction(Intent.ACTION_SCREEN_OFF);
            context.registerReceiver(mBatteryLevelReceiver, batteryFilter);
        }
        //add by sunxiaolong@wind-mobi.com for reverse charging end
    ...
    private void takeScreenshot(boolean sendBroadcast) {
        synchronized (mScreenshotWithBroadcastLock) {
            if (mScreenshotWithBroadcastConnection != null) {
                return;
            }
    ...
    final Runnable mTakeScreenshotWithBroadcastRunnable = new Runnable() {
        @Override public void run() {
            takeScreenshot(true);
        }
    };
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
		...
		//lizusheng@wind-mobi.com 20160314 add for Emode start
	        if (win != null && win.getAttrs() != null){
                if ("com.wind.emode".equals(win.getAttrs().packageName)) {
                    return 0;
                }      
            }
		//lizusheng@wind-mobi.com 20160314 add for Emode end
    ...
    private void handleGestures(int keyCode) {
        String name = null;
        int index = 0;
        switch (keyCode) {
            case KeyEvent.KEYCODE_F13://c, F13
                index = 2;
                break;
            case KeyEvent.KEYCODE_F14://e, F14
                index = 3;
                break;
            case KeyEvent.KEYCODE_F15://w, F15
                index = 4;
                break;
            case KeyEvent.KEYCODE_F18://s, F18
                index = 5;
                break;
            case KeyEvent.KEYCODE_F19://z, F19
                index = 6;
                break;
            case KeyEvent.KEYCODE_F20://v, F20
                index = 7;
                break;
            case KeyEvent.KEYCODE_F21: //double click, F21
                index = 1;
                break;
        ...
        if (isGestureEnabled(index)) {
            ...
            mSensorManager.registerListener(mDistanceSensorListener, mProximityMotion, SensorManager.SENSOR_DELAY_FASTEST);
            Slog.i(TAG, "send MSG_GESTURE_LISTEN " + keyCode);
            Message msg = mHandler.obtainMessage(MSG_GESTURE_LISTEN, Integer.valueOf(keyCode));
            msg.setAsynchronous(true);
    ...
    private boolean isGestureEnabled(final int index) {
        String enabledList = Settings.System.getString(mContext.getContentResolver(), Settings.System.GESTURES_ENABLED);
        if (enabledList == null || enabledList.substring(0, 1).equals("0")) {
            return false;
        }
    ...
    private boolean isSensorEnabled() {
        ...
        if ("com.wind.emode".equals(packageName) || "com.asus.camera".equals(packageName)) {
            return true;
        }
        return false;
    }
    ...
    private void sendGestureBroadcast(int keycode){
        if (mContext != null) {
            Intent intent = new Intent("com.asus.launch_app_by_gesture");
            intent.putExtra("keyCode", keycode);
            //add by mohongwu@wind-mobi.com 2016/10/1 start
            intent.setPackage("com.android.settings");
            //add by mohongwu@wind-mobi.com 2016/10/1 end
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
        }
    }

    private boolean isGestureEnabled() {
        String opString = SystemProperties.get("persist.asus.gesture.type", "0000000");
    ...
    private final SensorEventListener mDistanceSensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (event.values != null) {
               ...
               //remove by mohongwu@wind-mobi.com 2016/9/9 start
               /* if (its[0] < distance) {
                    mGestureEnabled = false;
                } else {
                    mGestureEnabled = true;
                }*/
               //remove by mohongwu@wind-mobi.com 2016/9/9 end
    ...
    private void toggleRecentApps() {
        // wangyan add 2016/03/09 for Feature 98800 -s
        if (android.os.SystemProperties.get("ro.wind_os_app_windpowersaver").equals("1")) {
            if ("on".equals(android.provider.Settings.System.getString(mContext.getContentResolver(), "powersave_super"))) {
                Log.i("zhangxutong_homekey", "disable toggleRecentApps");
                return;
            }
        }
        // wangyan add 2016/03/09 for Feature 98800 -e
    ...
    public void layoutWindowLw(WindowState win, WindowState attached) {
        ...
        if (!isDefaultDisplay) {
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                // pengfugen@wind-mobi.com 2016/03/24 start
                else if (WIND_DEF_ONE_HAND_CONTROL && attrs.type == WindowManager.LayoutParams.TYPE_MAGNIFICATION_OVERLAY
    ...
    public void notifyLidSwitchChanged(long whenNanos, boolean lidOpen) {
        ...
        // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -s
        boolean isPrivateUser = (android.os.Build.FEATURES.HAS_TRANSCOVER ? isPrivateUser() : false);
        // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -e
        ...
        if (lidOpen) {
            ...
            // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -s
        } else if (WIND_DEF_ASUS_FLIPCOVER) {
            if(android.os.Build.FEATURES.HAS_TRANSCOVER&&mSystemBooted&&((mHasTranscoverInfoFeature&&mIsTranscoverEnabled&&mTranscoverAutomaticUnlock)||isPrivateUser)) {
        ...
        } else if (!mLidControlsSleep) {//aosp
            mPowerManager.userActivity(SystemClock.uptimeMillis(), false);
        }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
		...
		// wangyan@wind-mobi.com add 2016/04/02 for Feature #100625 -s
        if (down == true && WIND_GESTURES_SUPPORTED) {
            handleGestures(keyCode);
        }
        // wangyan@wind-mobi.com add 2016/04/02 for Feature #100625 -e
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
				...
				/* lifeifei@wind-mobi.com 20160407 add for 104994 begin */
                //BEGIN: Jeffrey_Chiang@asus.com Instant camera porting
                if (mIsInstantCameraEnabled && !interactive && mLidState != LID_CLOSED && !isMusicActive()) {
            ...
            //add by sunxiaolong@wind-mobi.com for otg reverse charging 20160520 start
            case KeyEvent.KEYCODE_F11:
                if (WIND_OTG_REVERSE) {
                    result &= ~ACTION_PASS_TO_USER;
                    if (down && !POP_OTG) {
                        Message otgMsg = Message.obtain();
                        otgMsg.what = OTG_PLUG_IN_OPERATION;
                        mHandler.sendMessage(otgMsg);
                        POP_OTG = true;
                    }
                }
                break;
            case KeyEvent.KEYCODE_F12:
                if (WIND_OTG_REVERSE) {
                    ...
                    if (down && POP_OTG) {
                        ...
                        Message otgOutMsg = Message.obtain();
                        otgOutMsg.what = OTG_PLUG_OUT_OPERATION;
                        mHandler.sendMessage(otgOutMsg);
                    }
                }
                break;
            //add by sunxiaolong@wind-mobi.com for otg reverse charging 20160520 end
            ...
            case KeyEvent.KEYCODE_F9: {
                if (isSensorEnabled()) {
                    // nothing
                }else if (WIND_DEF_ASUS_FLIPCOVER && down) {
                    notifyLidSwitchChanged(0, false);
                }
                break;
            }
	...
	/* lizusheng@wind-mobi.com add for AsusNavigation 20160512 start */
            ...
            case KeyEvent.KEYCODE_FINGERPRINT_DTAP: {
                Log.i(TAG, "KEYCODE_FINGERPRINT_DTAP TO start Camera");
				int mLaunchCameraMode = Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.ASUS_FINGERPRINT_SELFIE_CAMERA, 0);
				if(!isKeyguardLocked() && mLaunchCameraMode == 1 && !isSensorEnabled()){
				Log.i(TAG, "Start to open Camera when Screen is unlock");
				Intent intent = new Intent("com.asus.camera.action.STILL_IMAGE_BACK_CAMERA");
				intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
				mContext.startActivity(intent);	
				Log.i(TAG, "finish start");
				}else{
				Log.i(TAG, "Do nothing when Screen is lock ");
				}		
            } break;
	...
	/* lizusheng@wind-mobi.com add for AsusNavigation 20160512 end */
            ...
            case KeyEvent.KEYCODE_F19:      //z, F19
            case KeyEvent.KEYCODE_F20: {    //v, F20
                if (WIND_DEF_ASUS_GESTURES && down) {
                    //remove by mohongwu@wind-mobi.com 2016/9/9 start
                    /*if (mGestureEnabled) {
                        sendGestureBroadcast(keyCode);
                    }*/
    ...
    //add by sunxiaolong@wind-mobi.com for otg reverse charging start
    private void popUpDialog(Context mContext){
        CharSequence[] titleItems = new CharSequence[]{mContext.getText(R.string.usb_otg_normal_mode_title)
                ,mContext.getText(R.string.usb_otg_reverse_charging_mode_title)};

        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
        builder.setTitle(mContext.getText(R.string.usb_otg_title));
        builder.setSingleChoiceItems(titleItems,defaultPosition, new DialogInterface.OnClickListener() {
            @Override
        ...
        ReverseChargingDialog = builder.create();
        ReverseChargingDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
        ReverseChargingDialog.show();
    }

    private void chooseOtgMode(int witch){
        switch (witch){
            case 0:
                setPowerReverseChargeEnable(false,OTG_REVERSE_CHARGING_CONTROL);
                break;
    ...
    private boolean getOtgState(){
        try {
            FileInputStream fis = new FileInputStream("/sys/class/switch/otg_state/state");
            int result = fis.read();
            fis.close();
            return result != '0';
        }catch(Exception e){
    ...
    public static int checkBatteryPercent() {
        ...
        try {
            ...
            Log.e(TAG, "checkCharger: get battery capacity file err!", e);
        } finally {
            try {
    ...
    BroadcastReceiver mBatteryLevelReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            boolean chargeEnable = Settings.System.getInt(context.getContentResolver(),
                    Settings.System.ASUS_OTG_REVERSE_CHARGE_ENABLE, 1) == 1;
            if (getOtgState() && !POP_OTG) {
                POP_OTG = true;
            }
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                BATTERY_LEVEL = intent.getIntExtra(BatteryManager.EXTRA_LEVEL, 100);
                if (BATTERY_LEVEL <= 20 && POP_OTG) {
                    defaultPosition = 0;
                    setPowerReverseChargeEnable(false, OTG_REVERSE_CHARGING_CONTROL);
                }
            } else if (Intent.ACTION_SCREEN_ON.equals(action)) {
                if (POP_OTG && BATTERY_LEVEL > 20) {
                    setPowerReverseChargeEnable(true, OTG_CHARGING_ENABLE);
                }
            } else if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                if (POP_OTG && !chargeEnable && defaultPosition == 1) {
                    setPowerReverseChargeEnable(false, OTG_CHARGING_ENABLE);
                }
            }
        }
    ...
    public void screenTurnedOff() {
        ...
        // wangyan@wind-mobi.com add 2016/07/22 Feature #110195 start
        //remove by mohongwu@wind-mobi.com 2016/9/9 start
        /*if (false && isGestureEnabled()) {
            mSensorManager.registerListener(mDistanceSensorListener, mProximityMotion, SensorManager.SENSOR_DELAY_NORMAL);
        }*/
        //remove by mohongwu@wind-mobi.com 2016/9/9 end
        // wangyan@wind-mobi.com add 2016/07/22 Feature #110195 end
    }
    ...
    public void screenTurnedOn() {
        ...
        // wangyan@wind-mobi.com add 2016/07/22 Feature #110195 start
        //modify by mohongwu@wind-mobi.com 2016/9/9 start
        //remove by mohongwu@wind-mobi.com 2016/9/13 start
        /*if (isGestureEnabled()) {
            mSensorManager.unregisterListener(mDistanceSensorListener);
        }*/
        //remove by mohongwu@wind-mobi.com 2016/9/13 end
        //modify by mohongwu@wind-mobi.com 2016/9/9 end
        // wangyan@wind-mobi.com add 2016/07/22 Feature #110195 end
    }
    ...
    public void dismissKeyguardLw() {
        if (mKeyguardDelegate != null && mKeyguardDelegate.isShowing()) {
            ...
            mHandler.post(new Runnable() {
				...
				/* lifeifei@wind-mobi.com 20160407 add for 104994 begin */
                    //BEGIN: Jeffrey_Chiang@asus.com Instant camera porting
                    if ((mIsLongPressInstantCameraEnabled || mIsInstantCameraEnabled) && !isScreenOn() && !isKeyguardSecure()) {
    ...
    public void systemBooted() {
        ...
        // wangchaobin@wind-mobi.com add 2016/09/29  for Feature #132839 -s
        if(ZenMotionGesturesUtils.WIND_DEF_ASUS_GESTURES){
            ZenMotionGesturesUtils.initGesturesItem();
        }
        // wangchaobin@wind-mobi.com add 2016/05/29  for Feature #132839 -e
    }
    ...
    private void applyLidSwitchState() {
        // wangyan@wind-mobi.com add 2016/05/11  for Feature #110122 -s
        if (WIND_DEF_ASUS_FLIPCOVER && android.os.Build.FEATURES.HAS_TRANSCOVER) {
        ...
        } else if (mLidState == LID_CLOSED && mLidControlsSleep) {//aosp
            mPowerManager.goToSleep(SystemClock.uptimeMillis(),
                    PowerManager.GO_TO_SLEEP_REASON_LID_SWITCH,
                    PowerManager.GO_TO_SLEEP_FLAG_NO_DOZE);
        }
    ...
    private void launchInstantCamera(String src, String suspend, int keyCode) {
        ...
        intent.putExtra("instant_camera_keycode", Integer.toString(keyCode));
    ...
    private void holdInstantCameraWakeLock() {
        if (mInstantCameraWakeLock == null) {
            mInstantCameraWakeLock = mPowerManager.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "instantCamera");
        }

        if (!mInstantCameraWakeLock.isHeld()) {
            Slog.d(TAG, "InstantCamera : Acquire instant camera wakelock for 500 ms!");
            mInstantCameraWakeLock.acquire(500L);
        }
    }
    ...
    private final Runnable mCameraKeyDoubleClickTimeoutRunnable = new Runnable() {
        @Override
        public void run() {
            if (mCameraKeyDoubleClickPending) {
                mCameraKeyDoubleClickPending = false;
            }
        }
    };
    private final Runnable mRecordKeyDoubleClickTimeoutRunnable = new Runnable() {
        @Override
        public void run() {
            if (mRecordKeyDoubleClickPending) {
                mRecordKeyDoubleClickPending = false;
            }
        }
    };
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    //BEGIN: John_Ho@asus.com
    //Handle Long Press Recent Key Functions
    ...
    private boolean sendKeyEvent(int event) {
        ...
        try {
            KeyEvent down = new KeyEvent(now, now, KeyEvent.ACTION_DOWN, event, 0);
            KeyEvent up = new KeyEvent(now, now, KeyEvent.ACTION_UP, event, 0);
   ...
   ///@}
    ...
    //add by mohongwu@wind-mobi.com 2016/9/9 start
    private int getProximitySensorData(){
        File mProximsenor = new File("/sys/bus/platform/drivers/als_ps/pdata1");
        int data = 0;
        ...
        if (mProximsenor.exists()) {
            ...
            Slog.i(TAG, "MSG_GESTURE_LISTEN value " + data);
        }
        return data;
    }
    //add by mohongwu@wind-mobi.com 2016/9/9 end
